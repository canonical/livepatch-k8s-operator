...
class TestRedactFunction(unittest.TestCase):
    ...
    def test_word_boundary_prevents_false_positive(self):
        # "token_count" should not be redacted — token is not a whole word here
...
class TestRedactingFilter(unittest.TestCase):
    ...
    def test_filter_redacts_percent_style_args(self):
        # Each arg is redacted if it itself matches a pattern.
    ...
    def test_filter_redacts_dict_style_args(self):
        # Dict values that themselves contain a pattern are redacted.
        record = _make_record(
            "dsn: %(dsn)s",
